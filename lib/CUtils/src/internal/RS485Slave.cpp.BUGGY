 ...
 * - Baud: 250000
 * - Broadcast with data: [Addr=0][MsgType=0][Length>0][ExportData...][Checksum]
 * - Poll (no data):      [Addr=N][MsgType=0][Length=0] (NO CHECKSUM when Length=0!)
 * - Response with data:  [Length>0][MsgType=0][InputData...][Checksum]
 * - Response no data:    [0x00] (single byte, NO CHECKSUM!)
 *
 * CRITICAL PROTOCOL NOTE:
 *   When Length=0, there is NO checksum byte! The packet ends immediately.
 *   This applies to both master->slave polls AND slave->master responses.
 *   This matches Arduino's DcsBiosNgRS485Slave behavior exactly.
 */
        ...
        switch (rs485s_state) {
            ...
            case RS485SlaveState::RX_WAIT_LENGTH: {
                ...
                if (rs485s_rxLength > 0) {
                    ...
                    // ======================================================
                    // CRITICAL FIX: Length=0 means NO DATA and NO CHECKSUM!
            ...
            case RS485SlaveState::RX_SKIP_RESPONSE_LENGTH: {
                ...
                if (rs485s_skipLength == 0) {
                    // Other slave had no data (single 0x00 byte, no checksum)
            ...
            case RS485SlaveState::RX_SKIP_RESPONSE_MSGTYPE: {
                // Just skip this byte
                rs485s_skipLength--;  // Length includes msgtype
                if (rs485s_skipLength == 0) {
                    rs485s_state = RS485SlaveState::RX_SKIP_RESPONSE_CHECKSUM;
                } else {
...
bool RS485Slave_init() {
    ...
    if (RS485_SLAVE_ADDRESS < 1 || RS485_SLAVE_ADDRESS > 126) {
        debugPrintf("[RS485S] ❌ Invalid address %d (must be 1-126)\n", RS485_SLAVE_ADDRESS);
        return false;
    }
    ...
    // Set UART pins
    // RS485_EN_PIN: -1 = auto-direction, >=0 = manual GPIO control
    int en_pin = RS485_EN_PIN;  // Copy to avoid macro expansion issues
