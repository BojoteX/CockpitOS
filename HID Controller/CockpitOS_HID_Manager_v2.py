...
class DeviceRegistry:
    ...
    def get_by_poller(self, poller_id: int) -> List[DeviceState]:
        """Returns devices assigned to a specific poller."""
...
# ══════════════════════════════════════════════════════════════════════════════
# TX DISPATCHER (UDP → HID fan-out)
# ══════════════════════════════════════════════════════════════════════════════

class TxDispatcher:
    """
    Receives UDP frames, slices into HID reports, dispatches to worker pool.
    Single producer (UDP RX), multiple consumers (TX workers).
    """
    ...
    def __init__(self, registry: DeviceRegistry, ui_queue: queue.Queue,
        ...
        self.uiq = ui_queue
        self.queue: queue.Queue = queue.Queue(maxsize=TX_QUEUE_SIZE)
        ...
        self.jobs_dropped = 0
        self.worker_busy = [False] * num_workers  # For UI status
    ...
    def dispatch(self, udp_data: bytes):
        """Called by UDP RX thread - slices data and enqueues."""
        # Pre-slice into HID reports (done once, shared by all workers)
        reports = []
        offset = 0
    ...
    def _worker_loop(self, worker_id: int):
        """Worker thread - pulls jobs, writes to all ready devices."""
        while self._running.is_set():
            ...
            # Fan out to all ready devices
            devices = self.registry.get_ready()
...
class RxPoller:
        ...
        while self._running.is_set():
            ...
            for dev_state in devices:
                ...
                try:
                    ...
                    # The firmware sends an Input Report to signal data is ready
                    data = dev_state.dev.read(DEFAULT_REPORT_SIZE, timeout_ms=0)
                    
                    # CRITICAL: Only drain Feature Reports if we received a trigger!
                    # This matches the firmware's trigger-based protocol where
                    # HIDManager_dispatchReport(true) signals pending commands.
                    if data:
                        self._drain_feature_reports(dev_state)
    ...
    def _drain_feature_reports(self, dev_state: DeviceState):
        """
        Read ALL pending Feature Reports (DCS commands from device).
        
        The firmware's ring buffer can hold up to 32 messages (DCS_USB_RINGBUF_SIZE).
        We drain exhaustively until empty to ensure no commands are left behind,
        which is especially important for rapid encoder rotations or burst inputs.
        
        Safety cap at 64 iterations (2x buffer size) prevents infinite loops
        on pathological firmware behavior.
        """
        # Firmware buffer is 32 slots; cap at 2x for safety margin
        ...
        for _ in range(MAX_DRAIN_ITERATIONS):
            try:
                ...
                # Empty buffer = all zeros → we've drained everything
                if not any(payload):
                    break
                
                msg = payload.rstrip(b'\x00').decode(errors='replace').strip()
                
                # Skip empty strings and handshake echoes
        ...
        # Log if we hit the safety cap (indicates potential firmware issue)
...
class HandshakeManager:
    ...
    def _handshake_loop(self):
        while self._running.is_set():
            ...
            if self._do_handshake(dev_state):
                ...
                # Try to enable non-blocking mode
                try:
    ...
    def _clear_backlog(self, dev_state: DeviceState) -> bool:
        ...
        This is critical for clean reconnection - without it, old commands
        from a previous session could be sent to DCS.
        """
        dev = dev_state.dev
        max_attempts = 100
...
class HotplugMonitor:
    ...
    def _handle_connect(self, info: dict):
        ...
        if self.registry.add(dev_state):
            self.uiq.put(('log', dev_state.name, f"Connected (reconnect #{dev_state.reconnections})"))
            self.uiq.put(('status', dev_state.name, None))
...
class CockpitHidBridge:
    ...
    def start(self):
        ...
        # Create UI first (need its queue for other components)
        # Temporary placeholder - we'll set real values after creating other components
        self.rx_pollers = []
        
        # TX dispatcher
        self.tx_dispatcher = TxDispatcher(
            self.registry,
            queue.Queue(),  # Temporary, will be replaced
            num_workers=TX_WORKERS
        )
        
        # UDP network
        self.udp = UdpNetwork(self.tx_dispatcher, queue.Queue())
        ...
        for i in range(RX_POLLERS):
            poller = RxPoller(
                i, self.registry,
                queue.Queue(),
                on_command=lambda cmd: None  # Placeholder
            )
            self.rx_pollers.append(poller)
        
        # Now create the real UI with all components
        self.ui = ConsoleUI(
            self.registry,
            self.udp,
            self.tx_dispatcher,
        ...
        # Re-wire all components to use UI queue
        self.tx_dispatcher.uiq = self.ui.uiq
        self.udp.uiq = self.ui.uiq
        for poller in self.rx_pollers:
            poller.uiq = self.ui.uiq
