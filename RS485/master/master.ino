...
// ============================================================================
// 1 = ISR-driven (lowest latency, like AVR) - RX interrupt on every byte
// 0 = Driver-based (portable fallback) - uses ESP-IDF UART driver with DMA
...
// ============================================================================
// DCS-BIOS PROTOCOL PARSER (for LED sync verification)
// ============================================================================
...
// Forward declaration for ISR (only needed in ISR mode)
class RS485Master;
#if USE_ISR_MODE
...
private:
    ...
    inline void setDE(bool high) {
        if (dePin >= 0) {
            gpio_set_level((gpio_num_t)dePin, high ? 1 : 0);
        }
    }
#endif
    ...
    inline void finishTransmit() {
        ...
        uart_wait_tx_done((uart_port_t)uartNum, pdMS_TO_TICKS(10));
    ...
    inline void txWriteBlocking(const uint8_t* data, uint16_t len) {
        uart_write_bytes((uart_port_t)uartNum, (const char*)data, len);
    }
#endif
...
public:
    ...
    // ========================================================================
    // init() - UART setup (ISR mode or Driver mode based on USE_ISR_MODE)
    // ========================================================================

    void init() {
        dbgPrintf("  Bus UART%d: TX=%d RX=%d DE=%d (%s)\n", uartNum, txPin, rxPin, dePin,
...
#if USE_ISR_MODE
        // =====================================================================
        // ISR MODE: Bare-metal UART setup (from slave ISR-mode pattern)
        // =====================================================================
...
#else
        // =====================================================================
        // DRIVER MODE: ESP-IDF UART driver setup (DMA-backed, portable)
        // =====================================================================

        dbgPrint("  Installing UART driver...");
        dbgFlush();
        ...
        ESP_ERROR_CHECK(uart_driver_install((uart_port_t)uartNum, 256, 256, 0, NULL, 0));
        ESP_ERROR_CHECK(uart_param_config((uart_port_t)uartNum, &uart_config));
        ESP_ERROR_CHECK(uart_set_pin((uart_port_t)uartNum, txPin, rxPin,
                                      UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
        ...
        dbgPrint("  UART driver installed");
    ...
    // ========================================================================
    // processRxByte() - processes one received byte through state machine
    // Used by both ISR mode (from ISR) and Driver mode (from loop polling)
    // ========================================================================

    inline void processRxByte(uint8_t c) {
        switch (state) {
...
#else
    // ========================================================================
    // pollRx() - Driver mode: read bytes from UART driver and process
    // Called from loop() instead of ISR
    // ========================================================================

    void pollRx() {
        uint8_t rxBuf[64];
        int len = uart_read_bytes((uart_port_t)uartNum, rxBuf, sizeof(rxBuf), 0);
        for (int i = 0; i < len; i++) {
    ...
    void loop() {
        ...
        if (state == STATE_RX_WAIT_MSGTYPE ||
            ...
            if ((now - rxStartTime) > RX_TIMEOUT_US) {
                ...
                // Flush any remaining bytes from RX
#if USE_ISR_MODE
                uart_ll_rxfifo_rst(uartHw);
                uart_ll_clr_intsts_mask(uartHw, UART_INTR_RXFIFO_FULL);
                uart_ll_ena_intr_mask(uartHw, UART_INTR_RXFIFO_FULL);
 ...
 *
 * Also feeds the local parser for LED sync verification (if enabled).
 */
static void drainPCInput() {
    while (pcRxBuffer.isNotEmpty()) {
        uint8_t c = pcRxBuffer.get();

        // Feed local parser for LED sync (master mirrors slave LED output)
        #if MASTER_MC_READY_PIN >= 0
...
void setup() {
    ...
    dbgPrint("");
    dbgPrint("===========================================");
    dbgPrintf("ESP32 RS485 Master v2 - %s\n", USE_ISR_MODE ? "ISR Mode" : "Driver Mode");
...
#if MASTER_MC_READY_PIN >= 0
    dbgPrintf("LED sync enabled on GPIO %d (mirrors slave MC_READY)\n", MASTER_MC_READY_PIN);
    UDP_DBG("LED sync enabled on GPIO %d", MASTER_MC_READY_PIN);
#endif
}
...
void loop() {
    ...
    // Update LED output (mirrors slave for sync verification)
    #if MASTER_MC_READY_PIN >= 0
    ...
    while (bus != nullptr) {
#if !USE_ISR_MODE
        // Driver mode: poll for RX bytes (ISR mode handles this in interrupt)
        bus->pollRx();
#endif
...
// ============================================================================
/**
 * ARCHITECTURE: AVR-ALIGNED WITH DUAL-MODE SUPPORT + V2 IMPROVEMENTS
 * ===================================================================
 *
 * Two operating modes selected at compile time via USE_ISR_MODE:
 *
 *   USE_ISR_MODE 1 (default):
 *     - UART RX interrupt fires immediately when byte arrives
 *     - State machine runs in ISR context - lowest latency
 *     - Uses bare-metal uart_ll_* functions for direct hardware access
 *     - Matches AVR's USART_RX_vect behavior exactly
 *
 *   USE_ISR_MODE 0 (fallback):
 *     - Uses ESP-IDF UART driver with DMA-backed 256-byte RX buffer
 *     - State machine runs in main loop via pollRx()
 *     - Higher latency but better buffering during TX operations
 *     - Use this if ISR mode shows timing issues or missed bytes
 *
