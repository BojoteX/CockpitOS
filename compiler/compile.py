...
# Options that CockpitOS cares about (key -> display name)
IMPORTANT_OPTIONS = [
    ("USBMode",         "USB Mode"),
...
# All five defines that participate in the transport assert
ALL_TRANSPORT_DEFINES = ["USE_DCSBIOS_SERIAL", "USE_DCSBIOS_USB", "USE_DCSBIOS_WIFI", "USE_DCSBIOS_BLUETOOTH", "RS485_SLAVE_ENABLED"]

# Which single define goes to 1 for each transport selection
TRANSPORT_KEY = {
...
def read_current_transport():
    """Determine which transport is active from Config.h.
    RS485 Slave IS a transport (it's in the assert sum)."""
    if read_config_define("RS485_SLAVE_ENABLED") == "1":
        return "slave"
    elif read_config_define("USE_DCSBIOS_SERIAL") == "1":
        return "serial"
    elif read_config_define("USE_DCSBIOS_USB") == "1":
        return "usb"
    elif read_config_define("USE_DCSBIOS_WIFI") == "1":
        return "wifi"
    elif read_config_define("USE_DCSBIOS_BLUETOOTH") == "1":
        return "ble"
    return None

def read_current_role():
    """Determine current device mode from Config.h."""
    master = read_config_define("RS485_MASTER_ENABLED")
    slave  = read_config_define("RS485_SLAVE_ENABLED")
...
def configure_transport_and_role(prefs):
    ...
    if mode == "slave":
        ...
        current_addr = read_config_define("RS485_SLAVE_ADDRESS") or "1"
    ...
    for define in ALL_TRANSPORT_DEFINES:
        target = 1 if define == active_key else 0
        old = read_config_define(define)
        if old != str(target):
            write_config_define(define, str(target))
    ...
    old_m = read_config_define("RS485_MASTER_ENABLED")
    if old_m != str(master_val):
        write_config_define("RS485_MASTER_ENABLED", str(master_val))
    ...
    if slave_address is not None:
        old = read_config_define("RS485_SLAVE_ADDRESS")
        if old != str(slave_address):
            write_config_define("RS485_SLAVE_ADDRESS", str(slave_address))
            changes.append(f"RS485_SLAVE_ADDRESS: {old} -> {slave_address}")

    # --- RS485 task config for master (auto-tune based on transport) ---
    if mode == "master":
        if transport == "wifi":
            write_config_define("RS485_USE_TASK", "0")
            changes.append("RS485_USE_TASK -> 0 (loop, best for WiFi)")
        else:
            write_config_define("RS485_USE_TASK", "1")
            write_config_define("RS485_TASK_CORE", "0")
            changes.append("RS485_USE_TASK -> 1, RS485_TASK_CORE -> 0")

    # --- USB mode override for board options ---
    usb_mode_needed = None
    if transport == "usb":
        usb_mode_needed = "default"

    # Save in prefs
    prefs["transport"] = transport if transport else "slave"
    prefs["role"] = mode
    prefs["usb_mode"] = usb_mode_needed
...
def configure_options(fqbn, prefs):
    ...
    # If USB transport, force USB-OTG mode
    if prefs.get("usb_mode") == "default":
        if "USBMode" in board_options:
            selected["USBMode"] = "default"  # USB-OTG (TinyUSB)
            info(f"{DIM}USB Mode -> USB-OTG (TinyUSB) (required for USB transport){RESET}")
    ...
    mode = pick("Board options mode:", [
        ...
        ("Guided - review important options", "guided"),
    ...
    # Guided or Full
    if mode == "guided":
        keys_to_show = [k for k, _ in IMPORTANT_OPTIONS if k in board_options and k not in selected]
    else:
        keys_to_show = [k for k in board_options if k not in selected]

    # Also let user override the locked-in defaults in Full mode
    if mode == "full":
...
def build_firmware_name():
    ...
    Convention:
      ...
      RS485 Master: RS485-Master_LABEL_Transport.bin  e.g. RS485-Master_MAIN_USB.bin
      RS485 Slave:  RS485-Slave_LABEL.bin       e.g. RS485-Slave_MAIN.bin
    """
    label = read_active_label_set() or "UNKNOWN"
    ...
    if role == "slave":
        return f"RS485-Slave_{label}"
    elif role == "master":
        t = transport_names.get(transport, transport or "Unknown")
        return f"RS485-Master_{label}_{t}"
    else:
        t = transport_names.get(transport, transport or "Unknown")
        return f"{label}_{t}"
...
def compile_sketch(fqbn, options, verbose=False, export_bin=False):
    ...
    if proc.returncode == 0:
        ...
        # Find the .bin and rename to descriptive name
        fw_name = build_firmware_name()
        bin_info = None
        for f in BUILD_DIR.iterdir():
            if f.suffix == ".bin" and "merged" not in f.name and f.name.startswith("CockpitOS"):
                dest = BUILD_DIR / f"{fw_name}.bin"
                shutil.copy2(f, dest)
                bin_info = f"Binary: {fw_name}.bin ({dest.stat().st_size:,} bytes)"
                break
...
def show_config(prefs):
    ...
    if role == "master":
        smart = read_config_define("RS485_SMART_MODE") or "0"
        max_addr = read_config_define("RS485_MAX_SLAVE_ADDRESS") or "127"
        use_task = read_config_define("RS485_USE_TASK") or "?"
        task_core = read_config_define("RS485_TASK_CORE") or "?"
        info(f"  RS485 Smart Mode:  {smart}")
        info(f"  Max Slave Address: {max_addr}")
        info(f"  Use Task: {use_task}  Core: {task_core}")
    elif role == "slave":
        addr = read_config_define("RS485_SLAVE_ADDRESS") or "?"
        isr = read_config_define("RS485_USE_ISR_MODE") or "?"
        info(f"  Slave Address: {addr}")
        info(f"  ISR Mode: {isr}")

    # RS485 pins
    if role in ("master", "slave"):
        tx = read_config_define("RS485_TX_PIN") or "?"
        rx = read_config_define("RS485_RX_PIN") or "?"
        de = read_config_define("RS485_DE_PIN") or "?"
    ...
    # Debug flags
    debug = read_config_define("DEBUG_ENABLED") or "0"
    verbose = read_config_define("VERBOSE_MODE") or "0"
...
def do_compile(prefs, verbose=False):
    ...
    # Apply USB mode override if needed
    if prefs.get("usb_mode") == "default" and "USBMode" in opts:
        opts["USBMode"] = "default"
        prefs["options"] = opts
        save_prefs(prefs)
