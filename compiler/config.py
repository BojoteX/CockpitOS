...
# -----------------------------------------------------------------------------
# Version stamping — git describe → version.h
# -----------------------------------------------------------------------------
VERSION_H = SKETCH_DIR / "version.h"
...
def _resolve_version():
    """Derive version string from git state.

    Uses the latest semver tag globally (not just reachable ones) to avoid
    the git-describe pitfall where tags on main are unreachable from dev.

    Returns a version string like:
        "1.2.9"                 — HEAD is exactly on the tag
        "1.2.9+17.abc1234"     — 17 commits ahead of tag
        "1.2.9+17.abc1234.dirty" — with uncommitted changes
        "dev-abc1234"           — no semver tags at all
        "dev-nogit"             — git not available
    """
    # Latest semver tag across all branches
    tag = _git(["tag", "--sort=-v:refname", "-l", "v*"])
    if tag:
        tag = tag.splitlines()[0]   # first line = highest version

    sha = _git(["rev-parse", "--short", "HEAD"])
    if not sha:
        return "dev-nogit"

    # Dirty check (git diff --quiet exits 1 if dirty)
    try:
        r = subprocess.run(
            ["git", "diff", "--quiet", "HEAD"],
            capture_output=True, cwd=str(SKETCH_DIR), timeout=5
        )
        dirty = r.returncode != 0
    except Exception:
        dirty = False

    # No semver tag — bare hash
    if not tag or not re.match(r'^v?\d+\.\d+\.\d+$', tag):
        ver = f"dev-{sha}"
        return f"{ver}.dirty" if dirty else ver

    ver = tag.lstrip("v")

    # Distance from tag to HEAD (works across branches)
    count_str = _git(["rev-list", "--count", f"{tag}..HEAD"])
...
def generate_version_h():
    """Generate version.h from git state. Returns the version string.

    Always safe — no restore needed. The file is regenerated fresh each time,
    never committed to git, and Config.h has a __has_include fallback.

    If git is unavailable (e.g., release ZIP with no .git), preserves any
    existing version.h that was stamped by the release workflow.
    """
    version = _resolve_version()

    # Don't overwrite a release-stamped version.h with "dev-nogit"
    if version == "dev-nogit" and VERSION_H.exists():
        try:
            existing = VERSION_H.read_text(encoding="utf-8")
            m = re.search(r'VERSION_CURRENT\s+"(.+?)"', existing)
            if m and not m.group(1).startswith("dev-"):
                return m.group(1)  # keep the release stamp
        except OSError:
            pass

    content = (
        "// Auto-generated by CockpitOS compile tool - do not edit\n"
        "#pragma once\n"
        f'#define VERSION_CURRENT "{version}"\n'
    )
    try:
        VERSION_H.write_text(content, encoding="utf-8")
    except OSError:
        pass  # Non-fatal — __has_include fallback covers this

    return version


def get_version_string():
    """Get the current version string from git without writing version.h.
    For display purposes only (e.g., compiler menu).
    """
