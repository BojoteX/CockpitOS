...
def _parse_git_describe(raw):
    """Parse git describe output into a clean version string.

    Examples:
        "v1.2.9"                    → "1.2.9"
        "v1.2.9-14-gabc1234"       → "1.2.9+14.abc1234"
        "v1.2.9-14-gabc1234-dirty" → "1.2.9+14.abc1234.dirty"
        "abc1234"                   → "dev-abc1234"
    ...
    if dirty:
        raw = raw[:-6]  # strip "-dirty"

    # Try semver tag format: v1.2.3 or v1.2.3-N-gHASH
    ...
    # No semver tag — just a commit hash
    ver = f"dev-{raw}"
    if dirty:
        ver += ".dirty"
    return ver
...
def generate_version_h():
    """Generate version.h from git describe. Returns the version string.
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            version = _parse_git_describe(result.stdout)
        else:
            version = "dev-nogit"
