...
The system supports up to 16 simultaneous CoverGate definitions. The `kCoverGates[]` array and `kCoverGateCount` are defined per-label-set in `CoverGates.h` inside the label set folder (e.g., `src/LABELS/LABEL_SET_YOURPANEL/CoverGates.h`). These files are managed by the **CoverGate Editor** in the Label Creator and included via `LabelSetSelect.h`. The maximum count is defined in `Mappings.h`.
