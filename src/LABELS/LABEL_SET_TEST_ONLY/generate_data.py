...
with open(INPUT_REFERENCE, "w", encoding="utf-8") as f2:
    ...
    f2.write("    int8_t     port;           // Port index (For PCA use port 0 or 1, for GPIO use PIN and use -1 when sharing GPIOs to differentiate HIGH/LOW)\n")
    f2.write("    int8_t     bit;            // Bit position is used for PCA, GPIO also uses it but ONLY for one-hot selectors (GPIO assigned for each position) in such cases set -1 as port\n")
