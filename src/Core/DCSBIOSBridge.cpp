...
// #endif
    ...
    // Self-aligned sync: for 2-pos switches, suppress sending a value DCS already
    // reports. Emulates magnetic switch behavior in software — when DCS auto-releases
    // a switch, CockpitOS acknowledges the new state without sending a redundant command.
    // Skipped during force sync (panel init must always push through) and unknown state.
    if (!force && value <= 1 && e->lastValue != 0xFFFF && e->lastValue == value) {
