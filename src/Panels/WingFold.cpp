...
// Minimum spacing between ANY two DCS commands from this panel
static constexpr uint32_t MIN_CMD_SPACING_MS = 500;
...
// Simple fixed-size command queue to enforce spacing
struct PendingCmd {
    const char* label;
    bool        forceSend;
    bool        value;
...
// Queue helper: enqueue a single DCS command (label/value) to be sent later with spacing
static void enqueueCmd(const char* label, bool forceSend, bool value) {
    uint8_t nextTail = (uint8_t)((cmdTail + 1u) % (uint8_t)4u);
    if (nextTail == cmdHead) {
        // queue full, drop oldest
        cmdHead = (uint8_t)((cmdHead + 1u) % (uint8_t)4u);
    }
    cmdQueue[cmdTail].label = label;
    cmdQueue[cmdTail].forceSend = forceSend;
    cmdQueue[cmdTail].value = value;
    cmdTail = nextTail;
}

// Emit both axial + angle by queuing them; actual send happens in loop with spacing
static void emitState(bool forceSend) {
    // axial
    enqueueCmd((curS.ax == PULL) ? L_PULL_POS1 : L_PULL_POS0, forceSend, true);

    // angle
    const char* angLab =
        (curS.ang == FOLD_POS) ? L_ROT_FOLD :
        (curS.ang == HOLD_POS) ? L_ROT_HOLD :
        L_ROT_UNFOLD;

    enqueueCmd(angLab, forceSend, true);
}
...
void WingFold_init() {
    ...
    // Queue both commands as init/sync; loop will enforce spacing between them
    emitState(true);
}
...
void WingFold_loop() {
    ...
    // --- Global send pacing: never send two commands < MIN_CMD_SPACING_MS apart ---
    if (cmdHead != cmdTail && el(now, lastCmdMs) >= MIN_CMD_SPACING_MS) {
        const PendingCmd& c = cmdQueue[cmdHead];
        HIDManager_setNamedButton(c.label, c.forceSend, c.value);
        cmdHead = (uint8_t)((cmdHead + 1u) % (uint8_t)4u);
        lastCmdMs = now;
    }
    ...
    if (curS.ax != prevS.ax || curS.ang != prevS.ang) {
        // Runtime edge: queue both commands; pacing is global
        emitState(false);
        prevS = curS;
    }
}
