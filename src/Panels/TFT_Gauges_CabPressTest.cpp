...
#if defined(HAS_CABIN_PRESSURE_GAUGE) && (defined(ESP_FAMILY_S3) || defined(ESP_FAMILY_S2)) && (defined(ENABLE_TFT_GAUGES) && (ENABLE_TFT_GAUGES == 1))
...
// ═══════════════════════════════════════════════════════════════════════════════
// QSPI mode (Waveshare ESP32-S3-LCD-1.85 and similar all-in-one boards):
//   Uses 4 data lines (IO0–IO3), no DC pin. Set CABIN_PRESSURE_USE_QSPI = 1.
//
// SPI mode (external panel with separate wiring):
//   Uses MOSI + DC pin (classic 4-wire SPI). Set CABIN_PRESSURE_USE_QSPI = 0.
//
// Override any pin from your label set / board config header before this file.
// ═══════════════════════════════════════════════════════════════════════════════
...
// --- Shared pins (both modes) ---
  ...
  #define CABIN_PRESSURE_CS_PIN   21              // Chip Select
#endif
#ifndef CABIN_PRESSURE_SCLK_PIN
  #define CABIN_PRESSURE_SCLK_PIN 40              // Clock (PCLK)
#endif
#ifndef CABIN_PRESSURE_RST_PIN
  #define CABIN_PRESSURE_RST_PIN  -1              // On TCA9554 I2C expander, not direct GPIO
#endif
#ifndef CABIN_PRESSURE_BL_PIN
  #define CABIN_PRESSURE_BL_PIN   5               // Backlight PWM (via transistor Q3)
#endif

#if CABIN_PRESSURE_USE_QSPI
  // --- QSPI data pins (Waveshare ESP32-S3-LCD-1.85 — verified working) ---
  #ifndef CABIN_PRESSURE_IO0_PIN
    #define CABIN_PRESSURE_IO0_PIN  46            // DATA0
  #endif
  #ifndef CABIN_PRESSURE_IO1_PIN
    #define CABIN_PRESSURE_IO1_PIN  45            // DATA1
  #endif
  #ifndef CABIN_PRESSURE_IO2_PIN
    #define CABIN_PRESSURE_IO2_PIN  42            // DATA2
  #endif
  #ifndef CABIN_PRESSURE_IO3_PIN
    #define CABIN_PRESSURE_IO3_PIN  41            // DATA3
  #endif
#else
  // --- SPI pins (external panel, classic 4-wire) ---
  #ifndef CABIN_PRESSURE_DC_PIN
    #define CABIN_PRESSURE_DC_PIN   -1            // Data/Command
  #endif
  #ifndef CABIN_PRESSURE_MOSI_PIN
    #define CABIN_PRESSURE_MOSI_PIN -1            // MOSI / SDA
  #endif
  #ifndef CABIN_PRESSURE_MISO_PIN
    #define CABIN_PRESSURE_MISO_PIN -1            // MISO (unused)
  #endif
#endif
...
public:
    LGFX_CabPress() {
        ...
        // ── Backlight (PWM) ────────────────────────────────────────────────
        {
            ...
            lcfg.pin_bl   = CABIN_PRESSURE_BL_PIN;
            lcfg.invert   = false;
            lcfg.freq     = 12000;                // 12 kHz PWM — inaudible, good for LEDs
            lcfg.pwm_channel = 7;                 // Use high channel to avoid conflicts
            _light.config(lcfg);
...
void CabinPressureGauge_init()
{
    ...
    tft.setBrightness(255);                       // Backlight full on via Light_PWM
    ...
    debugPrintf(
        ...
        CABIN_PRESSURE_IO1_PIN,
        CABIN_PRESSURE_IO2_PIN,
        CABIN_PRESSURE_IO3_PIN,
        CABIN_PRESSURE_RST_PIN,
        CABIN_PRESSURE_BL_PIN
    );
#else
    debugPrintf(
        "✅ Cabin Pressure Gauge initialized (SPI mode @ %u MHz) "
        "DC=%d, CS=%d, MOSI=%d, SCLK=%d, RST=%d, BL=%d\n",
        (unsigned)(freq_write / 1000000u),
