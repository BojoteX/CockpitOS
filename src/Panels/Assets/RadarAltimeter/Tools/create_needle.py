...
# --- CONFIG ---
PNG_FILE = "cabinPressNeedle.png"
HEADER_FILE = "cabinPressNeedle.h"
ARRAY_NAME = "cabinPressNeedle"
EXPECTED_WIDTH = 23
EXPECTED_HEIGHT = 238
PIXELS_PER_LINE = 15

TRANSPARENT_MASK = 0x0120  # Value used to indicate transparency (RGB565)
SWAP_ENDIAN = False         # <--- Set to True for little-endian (MCU order), False for "PC order"
SWAP_ENDIAN_TRANSPARENT = False         # <--- Set to True for little-endian (MCU order), False for "PC order"
...
def write_header_file(pixels, output_path):
    with open(output_path, 'w') as f:
        f.write(f"// 'Needle', {EXPECTED_WIDTH}x{EXPECTED_HEIGHT} Pivot point 12x165\n")
