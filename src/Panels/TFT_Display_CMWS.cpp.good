...
static void CMWSDisplay_draw(bool force = false) {
    ...
    // If the device is OFF, blank once from the draw context only (never from callbacks).
    if (g_deviceOff) {
        if (g_dirty) {
            tft.startWrite();
            tft.fillScreen(COL_BLACK);
...
static void setLargeStateFromBits(int idx) {
    ...
    // Always update pending state (even OFF) under the mux.
    portENTER_CRITICAL(&g_stateMux);
    g_pending.large[idx] = st;
    portEXIT_CRITICAL(&g_stateMux);

    if (st == ElemState::OFF) {
        // IMPORTANT: never touch the TFT from callbacks (concurrency hazard).
        // Signal the draw task to blank once and stop rendering until ON again.
        if (!g_deviceOff) {
            g_deviceOff = true;
            g_forceFull = true;   // ensure full redraw when device comes back
            g_dirty = true;       // triggers a one-time blank in draw task
        }
        return;
    }
...
static void onPage(const char*, const char* value) {
    ...
    if (g_pending.page != newPage) {
        ...
        // IMPORTANT: never touch the TFT from callbacks (concurrency hazard).
        // Let the draw task handle blanking on page=0.
        g_deviceOff = (newPage == 0);

        g_forceFull = true;
        g_dirty = true;
        debugPrintf("[CMWS] Page changed to: %s", value ? value : "(null)");
        return;
    }
    portEXIT_CRITICAL(&g_stateMux);
}
