...
#define INTERFRAME_GAP_US   600      // 600µs silence between frames for slave sync
...
static void txTask(void* param) {
    ...
    while (true) {
        ...
        if (xQueueReceive(txQueue, &request, portMAX_DELAY) == pdTRUE) {
            ...
            // Provide inter-frame silence for slave sync
            if (INTERFRAME_GAP_US > 0) {
...
static void processMaster() {
    ...
    switch (masterState) {
        case STATE_IDLE:
            ...
            // Priority 2: Broadcast export data if we have any (unless poll interval elapsed)
