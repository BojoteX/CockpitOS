 ...
 *   - Inline blocking TX: TX blocks loop() briefly (no FreeRTOS TX task).
 ...
 *     even while main loop blocks during TX.
 *   - Inline PC forwarding: Slave responses forwarded to PC immediately after
 *     each bus loop(), reducing latency by one full cycle.
...
public:
    ...
    RS485Master(int uartNum, int txPin, int rxPin, int dePin)
        : uartNum(uartNum), txPin(txPin), rxPin(rxPin), dePin(dePin),
          ...
          statPolls(0), statResponses(0), statTimeouts(0), statBroadcasts(0),
          next(nullptr)
    {
        if (first == nullptr) {
            first = this;
    ...
    // CockpitOS rs485PollLoop order:
    //   1. Forward completed messages (done externally in Arduino loop)
    //   2. Drain timeout safety valve
    //   3. Check RX timeouts
    //   4. If IDLE: broadcast or poll
    void loop() {
        // Safety: drain timeout — if message sat complete too long, force-clear it
        ...
        // Check RX timeouts (non-IDLE states)
        if (state != STATE_IDLE) {
            int64_t now = esp_timer_get_time();
    ...
    void incResponses() { statResponses++; }  // Called from forwarding path (matches CockpitOS processCompletedMessage)
};
...
// Process completed slave responses — forward to PC
// Inlined in loop() to avoid Arduino IDE forward-declaration issues with class pointers
...
void loop() {
    ...
    // Catch-all: forward any pending slave responses to PC
    {
        RS485Master* b = RS485Master::first;
        while (b != nullptr) {
            if (b->messageBuffer.complete) {
                b->incResponses();
                uint8_t tmpBuf[MESSAGE_BUFFER_SIZE];
                size_t tmpLen = 0;
                while (b->messageBuffer.isNotEmpty() && tmpLen < sizeof(tmpBuf)) {
                    tmpBuf[tmpLen++] = b->messageBuffer.get();
                }
                if (tmpLen > 0) pcWrite(tmpBuf, tmpLen);
                b->messageBuffer.complete = false;
            }
            b = b->next;
        }
    }
    ...
    while (bus != nullptr) {
        ...
        // Immediate forwarding — slave response goes to PC right after detection
        if (bus->messageBuffer.complete) {
            bus->incResponses();
            uint8_t tmpBuf[MESSAGE_BUFFER_SIZE];
            size_t tmpLen = 0;
            while (bus->messageBuffer.isNotEmpty() && tmpLen < sizeof(tmpBuf)) {
                tmpBuf[tmpLen++] = bus->messageBuffer.get();
            }
            if (tmpLen > 0) pcWrite(tmpBuf, tmpLen);
    ...
    if (millis() - lastStatusMs >= STATUS_INTERVAL_MS) {
        ...
        while (bus != nullptr) {
            ...
            dbgPrintf("[MASTER] Polls=%lu Resp=%.1f%% Bcasts=%lu Timeouts=%lu Slaves=%d\n",
                      bus->getStatPolls(), respRate, bus->getStatBroadcasts(),
                      bus->getStatTimeouts(), onlineCount);
