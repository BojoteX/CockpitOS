#!/usr/bin/env python3
"""
CockpitOS BLE Latency Checker
==============================

Device-initiated RTT measurement for BLE HID gamepads.

PROTOCOL FLOW:
  1. User presses physical button on BLE device (LAT_TEST_PIN)
  2. Device captures T0 = esp_timer_get_time(), sets lat_t0_sent
  3. Device queues Feature Report with [MAGIC|seq|T0]
...
try:
    ...
    print()
    print("+----------------------------------------------------------------+")
    print("|  ERROR: Missing Required Module: hid                           |")
    print("+----------------------------------------------------------------+")
    print("|                                                                |")
    print("|  This tool requires 'hid' (hidapi) for HID communication.     |")
    print("|                                                                |")
...
# Input Report layout (from BidirectionalNew.h):
#   [0:31]  - 16x 16-bit axes (32 bytes)
#   [32:35] - 32 buttons (4 bytes) - bit 0 of byte 32 = latency test button
#   [36:63] - Padding (28 bytes) - bytes [36:39] = RTT result (uint32_t, us)
...
USAGE:
  ...
  5. Press Ctrl+C to stop and see statistics
    ...
    print()
    print("+----------------------------------------------------------------+")
    print("|            CockpitOS BLE Latency Checker                       |")
    print("+----------------------------------------------------------------+")

    try:
        dev, info = open_dev(args.vid, args.pid, args.product)
    except RuntimeError as e:
        print(f"|  ERROR: {e}".ljust(65) + "|")
        print("+----------------------------------------------------------------+")
    ...
    print(f"|  Device: VID=0x{info['vendor_id']:04X} PID=0x{info.get('product_id',0):04X}".ljust(65) + "|")
    print(f"|  Name: {info.get('product_string', 'N/A')[:54]}".ljust(65) + "|")
    print("+----------------------------------------------------------------+")
    print("|  DEVICE-INITIATED LATENCY TEST                                 |")
    print("|  Press the latency test button on your BLE device.             |")
    print("|  The device measures RTT; this script echoes the trigger.      |")
    ...
    if samples:
        print("+----------------------------------------------------------------+")
        print(f"|  STATISTICS ({len(samples)} samples)".ljust(65) + "|")
        print("+----------------------------------------------------------------+")
        ...
        if mean_rtt < 15:
            print("    [OK] Excellent - competitive gaming grade (<15ms RTT)")
        elif mean_rtt < 30:
            print("    [OK] Good - suitable for most gaming (<30ms RTT)")
        elif mean_rtt < 50:
        ...
        print("\n  No samples collected. Did you press the latency test button?\n")
