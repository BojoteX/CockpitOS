#!/usr/bin/env python3
"""
CockpitOS HID Manager v5.1 - Multi-Interface Multicast Fix
============================================================

Combines the PROVEN patterns from the original with clean code structure.
Now joins multicast on ALL network interfaces for universal compatibility.
...
5. NEW: Multicast join on all interfaces (fixes multi-NIC Windows issue)
...
REQUIRED_MODULES = {
    ...
    "ifaddr": "ifaddr",  # NEW: Required for multi-interface multicast
}
...
class NetworkManager:
    ...
    NEW in v5.1: Joins multicast on ALL network interfaces!
    """
    ...
    def __init__(self, uiq: queue.Queue, reply_addr_ref: list, get_devices_callback):
        ...
        self._joined_interfaces: List[str] = []  # Track which interfaces we joined
    ...
    def _join_multicast_all_interfaces(self, sock: socket.socket) -> List[str]:
        ...
        Returns list of successfully joined interface IPs.
        
        This is critical for Windows multi-NIC setups (e.g., wired + WiFi hotspot).
        """
        joined = []
    ...
    def _udp_rx_processor(self) -> None:
        ...
        try:
            ...
            # NEW: Join multicast on ALL interfaces
